...
def _render_opportunities_tab(listings_df, references_df, merged_df, price_history_df, favorites=None):
    ...
    if not opportunities.empty:
        ...
        if st.session_state["opp_view"] == "list":
            ...
            components.html(f"<html><body style='margin:0;padding:0'>{css}{table_html}</body></html>", height=height, scrolling=False)
        else:
            ...
            components.html(f"<html><body style='margin:0;padding:0'>{css}{cards_html}</body></html>", height=height, scrolling=False)
    else:
